...
set -e
echo "\n****************************\nHello! I'm the ShopWorx Kafka-sse-service Container. Starting up at: $(date)"
echo
echo "Setting kafka-sse-service configuration now ..."
echo
...
echo "Setting kafka-sse-service config ..."
sed -i "s|{{hostname}}|${brokerlist}|g" /home/emgda/shopworx/kafka-sse-service/config/config.json
chmod 0644 /home/emgda/shopworx/kafka-sse-service/config/config.json
echo "... done."
...
node home/emgda/shopworx/kafka-sse-service/index.js